...
const EventDetail = ({event, backClickHandler, showRegistrationModal, showRegistrationModalHandler, hideRegistrationModalHandler}) => {
  ...
  return (
    <Panel header={title}>
      ...
      <h2><FormattedDate value={event.date_start}/> (<FormattedRelative value={event.date_start}/>)</h2>
      <p>{event.excerpt}</p>
      { event.host ? (
        <small>Host: {event.host.prefix} {event.host.firstName} {event.host.lastName} ({event.host.jobTitle})</small>
      ) : null }
      <p className="text-right">
        <Button onClick={showRegistrationModalHandler} disabled={showRegistrationModal} bsStyle="primary"
